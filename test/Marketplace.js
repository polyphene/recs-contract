...
describe("Marketplace", function () {
    ...
    async function deployFixture() {
        ...
        return { marketplace, rec, owner, buyer, nobody };
    }

    async function deployAndList() {
        const payload = await deployFixture();

        await payload.rec.setApprovalForAll(payload.marketplace.address, true);

        const unitPriceWei = ethers.utils.parseEther("1.0");
        await payload.marketplace.list(payload.rec.address, 0, 10, unitPriceWei);
    ...
    describe("List", async function () {
        it("Should revert if listed amount is not positive", async function () {
            const { marketplace, rec } = await loadFixture(deployFixture);

            await expect(marketplace.list(rec.address, 0, 0, 1)).to.be.revertedWith(
                "Amount to be listed should be positive"
            );
        });

        it("Should revert if sender does not own listed amount", async function () {
            const { marketplace, rec, nobody } = await loadFixture(deployFixture);

            await expect(
                marketplace.connect(nobody).list(rec.address, 0, 10, 1)
        ...
        it("Should revert if marketplace has not management approval", async function () {
            ...
            await expect(marketplace.list(rec.address, 0, 10, 1)).to.be.revertedWith(
                "Marketplace should be approved to manage user tokens"
            );
        });

        it("Should list amount of token", async function () {
            const { marketplace, rec, owner } = await loadFixture(deployFixture);

            await rec.setApprovalForAll(marketplace.address, true);

            await marketplace.list(rec.address, 0, 10, 1);

            const listing = await marketplace.tokenListing(rec.address, 0);
        ...
        it("should emit TokenListed events", async function () {
            ...
            await rec.setApprovalForAll(marketplace.address, true);
            await expect(marketplace.list(rec.address, 0, 10, 1))
                .to.emit(marketplace, "TokenListed")
                .withArgs(rec.address, owner.address, 0, 10, 1);
        });
    });

    describe("Buy", async function () {
        it("Should revert if tokens are not listed", async function () {
            const { marketplace, rec, buyer } = await loadFixture(deployAndList);

            await expect(marketplace.connect(buyer).buy(rec.address, 1, 10)).to.be.revertedWith(
                "Token is not listed"
            );
        });
        ...
        it("Should revert if amount specified is too great", async function () {
            ...
            await expect(marketplace.connect(buyer).buy(rec.address, 0, 1000)).to.be.revertedWith(
        ...
        it("Should revert if buyer does not send enough ethers", async function () {
            ...
            await expect(marketplace.connect(buyer).buy(rec.address, 0, 10)).to.be.revertedWith(
                "Not enough ether to make the purchase"
            );
        });

        it("Should buy token", async function () {
            const { marketplace, rec, owner, buyer } = await loadFixture(deployAndList);

            // check token balance
            await expect(await rec.balanceOf(owner.address, 0)).to.be.equal(15);
            await expect(await rec.balanceOf(buyer.address, 0)).to.be.equal(0);
            // Check Eth balance
            ...
            const receipt = await (
                await marketplace.connect(buyer).buy(rec.address, 0, 10, { value: valueSent })
            ).wait();

            // check token balance
            await expect(await rec.balanceOf(owner.address, 0)).to.be.equal(5);
            await expect(await rec.balanceOf(buyer.address, 0)).to.be.equal(10);
            // Check Eth balance
        ...
        it("Should emit TokenBought events", async function () {
            const { marketplace, rec, owner, buyer, unitPriceWei } = await loadFixture(
                deployAndList
            );

            const valueSent = ethers.utils.parseEther("10.0");
            await expect(marketplace.connect(buyer).buy(rec.address, 0, 10, { value: valueSent }))
                .to.emit(marketplace, "TokenBought")
                .withArgs(rec.address, buyer.address, owner.address, 0, 10, unitPriceWei);
        });
    });
});
